use tdms_format::segment::LeadIn;
...
#[test]
fn can_read_lead_in() {
    let f = File::open(Path::new("data/standard.tdms")).expect("Failure to open file");
    let mut buffer: Vec<u8> = Vec::with_capacity(LeadIn::SIZE);
    f.take(LeadIn::SIZE as u64)
        .read_to_end(&mut buffer)
        .unwrap();
    let lead_in: LeadIn = match LeadIn::from_bytes(&mut buffer) {
        Ok(s) => s,
        Err(e) => panic!("{:?}", e),
    };

    assert_eq!(lead_in.tag, [84, 68, 83, 109]);
    assert_eq!(lead_in.version_number, 4713);
    assert_eq!(lead_in.next_segment_offset, 292862);
    assert_eq!(lead_in.raw_data_offset, 4862);
}
