...
#[cfg(feature = "tokio")]
#[derive(Debug)]
pub struct ChannelDataIter<'a, T, R: AsyncReadExt + AsyncSeekExt> {
    channel: RefCell<&'a Channel>,
    current_pos: RefCell<ChannelPositions>,
    segments: Vec<&'a Segment>,
    reader: BufReader<R>,
...
#[cfg(feature = "tokio")]
impl<'a, T, R: AsyncReadExt + AsyncSeekExt + std::marker::Unpin> ChannelDataIter<'a, T, R> {
    pub async fn new(
        segments: Vec<&'a Segment>,
        channel: &'a Channel,
        reader: BufReader<R>,
   ...
   #[async_recursion]
    async fn advance_reader_to_next(&mut self) -> Result<&Segment, TdmsError> {
        ...
        if stream_pos >= current_segment.end_pos {
            ...
            return self.advance_reader_to_next().await;
        }
...
#[cfg(feature = "tokio")]
#[async_trait]
impl<'a, R: AsyncReadExt + AsyncSeekExt> Stream for ChannelDataIter<'a, f64, R> {
    type Item = f64;
